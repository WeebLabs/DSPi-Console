...
final class ControlSurfacesWireTests: XCTestCase {
    ...
    // MARK: - Caps v18: auxiliary outputs (control_surfaces_aux_spec.md)
    //
    // Caps v17 (never shipped) kept aux outputs in a separate table of eight
    // pinless values.  Caps v18 replaced it: an aux output is a binding slot of
    // type CS_TYPE_AUX_OUT / CS_TYPE_AUX_PWM that owns its GPIO.

    /// The two component types append to the type table, so the caps header
    /// grows to 52 bytes (11 types) and the tail still has to be found after
    /// the table rather than at a fixed offset (§2.3).
    func testAuxTypeNumberingAndCapsV18Header() {
        XCTAssertEqual(CS_TYPE_DISPLAY, 8)           // last pre-v18 type, unmoved
        XCTAssertEqual(CS_TYPE_AUX_OUT, 9)
        XCTAssertEqual(CS_TYPE_AUX_PWM, 10)

        var d = Data([18, 16, 11, 70])   // caps 18, 16 bindings, 11 types, 70 nouns
        for _ in 0..<9 { d.append(contentsOf: [0xBC, 0x02, 1, CS_PINCLASS_ANY]) }
        // Both aux descriptors: no actions, one pin of any class.
        for _ in 0..<2 { d.append(contentsOf: [0x00, 0x00, 1, CS_PINCLASS_ANY]) }
        d.append(contentsOf: [16, 8, 8, 8])
        XCTAssertEqual(d.count, 52)
        guard let caps = CsCapsHeader.fromData(d) else { return XCTFail("caps parse failed") }
        XCTAssertEqual(caps.capsVersion, 18)
        XCTAssertEqual(caps.typeCount, 11)
        XCTAssertEqual(caps.maxIrCommands, 16)
        XCTAssertEqual(caps.maxMacroSteps, 8)
        XCTAssertEqual(caps.types.count, 11)
        XCTAssertEqual(caps.types[CS_TYPE_AUX_PWM], CsTypeDesc(actions: 0, pinCount: 1, pinClass: CS_PINCLASS_ANY))
        // The app's support gate: the table must reach the PWM type.
        XCTAssertGreaterThan(caps.typeCount, UInt8(CS_TYPE_AUX_PWM))
    }

    /// §2.1 / §2.2: an aux slot is an ordinary 24-byte binding whose byte 22,
    /// formerly reserved, carries the CS_AUX_X_* extras.  Pinned byte-exactly
    /// because a wrong offset would boot an amplifier trigger the wrong way.
    func testAuxSlotBindingLayout() {
        let relay = CsBinding(type: UInt8(CS_TYPE_AUX_OUT), flags: CS_FLAG_INVERT,
                              gpio0: 21, gpio1: CS_GPIO_UNUSED,
                              onDelay: 5, offDelay: 20, extras: CS_AUX_X_BOOT_ON)
        XCTAssertEqual(hex(relay.toData()),
            "09 00 00 01 15 ff 00 00 00 00 00 00 00 00 00 00 00 00 05 00 14 00 01 00")
        XCTAssertEqual(CsBinding.fromData(relay.toData()), relay)

        let lamp = CsBinding(type: UInt8(CS_TYPE_AUX_PWM), gpio0: 22, gpio1: CS_GPIO_UNUSED,
                             baseBright: 80, value: 12800,   // boots at 50 % (8.8)
                             extras: CS_AUX_X_BOOT_SAVED | CS_AUX_X_LINEAR)
        XCTAssertEqual(hex(lamp.toData()),
            "0a 00 00 00 16 ff 00 00 00 50 00 32 00 00 00 00 00 00 00 00 00 00 06 00")
        XCTAssertEqual(CsBinding.fromData(lamp.toData()), lamp)
        XCTAssertEqual(lamp.toData()[23], 0, "byte 23 is still reserved")
    }

    /// Every non-aux type must keep byte 22 at zero or the device rejects the
    /// binding, which is what keeps a pre-v18 config valid unchanged (§2.1).
    func testExtrasByteZeroByDefault() {
        XCTAssertEqual(CsBinding().toData()[22], 0)
        let button = CsBinding(type: UInt8(CS_TYPE_BUTTON), noun: UInt8(CS_NOUN_USER_MUTE),
                               action: UInt8(CS_ACT_TOGGLE), gpio0: 16, gpio1: CS_GPIO_UNUSED)
        XCTAssertEqual(button.toData()[22], 0)
    }

    /// §2.2: the three extras bits and their mask, and the 8.8 level ceiling.
    func testAuxExtrasBitsAndLevelCeiling() {
        XCTAssertEqual(CS_AUX_X_BOOT_ON, 0x01)
        XCTAssertEqual(CS_AUX_X_BOOT_SAVED, 0x02)
        XCTAssertEqual(CS_AUX_X_LINEAR, 0x04)
        XCTAssertEqual(CS_AUX_X_MASK, 0x07)
        XCTAssertEqual(CS_AUX_X_BOOT_ON | CS_AUX_X_BOOT_SAVED | CS_AUX_X_LINEAR, CS_AUX_X_MASK)
        XCTAssertEqual(CS_AUX_LEVEL_MAX_Q8, 25600, "100 % in 8.8")
    }

    /// §3: four commands in the low block, 0x02/0x03 gone, and the status code
    /// and target kind the spec fixes.  An aux apply reports as a plain binding
    /// slot, so there is no longer a tag of its own in the status channel.
    ...
    /// §7.1: a button toggling the relay in slot 1.  The target is the binding
    /// slot holding the aux output, not an aux index.
    func testAuxWiringExampleBytes() {
        let button = CsBinding(type: UInt8(CS_TYPE_BUTTON), noun: UInt8(CS_NOUN_AUX),
                               action: UInt8(CS_ACT_TOGGLE), gpio0: 16, gpio1: CS_GPIO_UNUSED,
                               event: CS_EVENT_PRESS, target: 1)
        XCTAssertEqual(hex(button.toData()),
            "01 44 04 00 10 ff 00 01 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00")
        XCTAssertEqual(CsBinding.fromData(button.toData()), button)

        let relay = CsBinding(type: UInt8(CS_TYPE_AUX_OUT), flags: CS_FLAG_INVERT,
                              gpio0: 21, gpio1: CS_GPIO_UNUSED)
        XCTAssertEqual(hex(relay.toData()),
            "09 00 00 01 15 ff 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00")
        XCTAssertEqual(CsBinding.fromData(relay.toData()), relay)
    }

    /// §7.2: an encoder stepping the lamp in slot 3 by 5 % (1280 in 8.8).  The
    /// level is 8.8 end to end in v18, so a fractional step is legal too.
    func testAuxLevelEncoderStep() {
        let encoder = CsBinding(type: UInt8(CS_TYPE_ENCODER), noun: UInt8(CS_NOUN_AUX_LEVEL),
                                action: UInt8(CS_ACT_STEP), gpio0: 18, gpio1: 19,
                                target: 3, step: csEncodeStep(5, unit: CS_UNIT_PERCENT))
        XCTAssertEqual(encoder.step, 1280)
        XCTAssertEqual(hex(encoder.toData()),
            "04 45 01 00 12 13 00 03 00 00 00 00 00 05 00 00 00 00 00 00 00 00 00 00")
        XCTAssertEqual(CsBinding.fromData(encoder.toData()), encoder)
        XCTAssertEqual(csEncodeStep(0.5, unit: CS_UNIT_PERCENT), 128)
    }

    /// §1.4: aux nouns take no group.  No group can carry the aux target kind -
    /// the group editor deals only in channel spaces - so a grouped aux binding
    ...
    /// §5: the notification is 9 bytes carrying the slot, both values (the
    /// level as 8.8 LE) and the dispatch source.  Decoded here exactly as
    /// InterruptMonitor decodes it.
    func testAuxNotificationLayout() {
        let packet: [UInt8] = [2, 0x0C, 0, 7, 3, 1, 0x00, 0x3C, PARAM_SRC_GPIO]   // slot 3 on, 60 %
        XCTAssertEqual(packet.count, 9)
        XCTAssertEqual(packet[1], 0x0C)
        XCTAssertEqual(Int(packet[4]), 3)
        XCTAssertTrue(packet[5] != 0)
        let level = UInt16(packet[6]) | (UInt16(packet[7]) << 8)
        XCTAssertEqual(level, 15360)
        XCTAssertLessThanOrEqual(level, CS_AUX_LEVEL_MAX_Q8)
        XCTAssertEqual(packet[8], PARAM_SRC_GPIO)
    }

    /// §3.3: the bulk read is 48 bytes, sixteen states then sixteen 8.8 LE
    /// levels, in slot order and not interleaved.  Getting that backwards would
    /// show every output at the wrong level with no error anywhere.
    func testAuxBulkStateReadOrder() {
        var payload = Data(count: 48)
        payload[0] = 1; payload[3] = 1                          // slots 0 and 3 on
        payload[CS_MAX_BINDINGS + 0] = 0x00; payload[CS_MAX_BINDINGS + 1] = 0x64   // slot 0: 100 %
        payload[CS_MAX_BINDINGS + 6] = 0x80; payload[CS_MAX_BINDINGS + 7] = 0x28   // slot 3: 40.5 %
        let states = (0..<CS_MAX_BINDINGS).map { payload[$0] != 0 }
        let levels = (0..<CS_MAX_BINDINGS).map { i -> UInt16 in
            let off = CS_MAX_BINDINGS + 2 * i
            return UInt16(payload[off]) | (UInt16(payload[off + 1]) << 8)
        }
        XCTAssertEqual(states.indices.filter { states[$0] }, [0, 3])
        XCTAssertEqual(levels[0], CS_AUX_LEVEL_MAX_Q8)
        XCTAssertEqual(levels[3], 10368)
        XCTAssertEqual(levels.indices.filter { levels[$0] != 0 }, [0, 3])
    }
}
